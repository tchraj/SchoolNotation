...
@section('content')
    @php
        $univs = App\Models\University::paginate(5);
        // $items = App\Models\University::paginate(5);
    @endphp
    {{-- <form method="POST" action="{{ route('univs.store') }}">
        @csrf
    ...
    <form class="w-full md:w-1/2 max-w-sm " method="POST" action="{{ route('univs.store') }}">
        @csrf
